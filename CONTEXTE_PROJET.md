...
| `interferences/` | Interférences — trous/fentes d'Young (modélisation 3D) & interférences d'ondes de surface (cuve à ondes) | Terminale | **Arborescence** | Dossier dupliqué depuis `diffraction/` puis adapté (même pipeline FFT et rendu 3D), mais **aucun fichier commun** : `sim.js` + `scene.js` (rendu 3D Three.js) + `surfaces.js` + `graph.js` + `principe.js` + `ui.js` ; 3 onglets — **Interférences en 1D** (mode 1D : deux haut-parleurs sur un axe et un micro déplaçable, trois bandes y₁ / y₂ / y₁+y₂, représentation en courbe de surpression ou en gaz de particules, λ et amplitudes réglables, enveloppe, repères d'interférences, cotes S₁M/S₂M et différence de marche, graphes temporels y(M,t) en fenêtres flottantes ; mode 2D en placeholder), **Ondes de surface** (2 sources ponctuelles synchrones vues de dessus, champ précalculé P/Q par géométrie, λ et écartement b réglables, sources activables séparément, distances S₁M/S₂M et différence de marche, zones d'interférences, vue plongeante 3D, graphe y(t), zoom et vitesse d'animation), **Ondes lumineuses** (figure d'interférences par FFT, enveloppe 3D du faisceau, fentes ou trous d'Young, écartement b 0,1–1 mm, mode lumière blanche, graphe I(x)) ; deep-linking via `#principe` / `#surfaces` / `#lumineuses` — voir `interferences/ARCHITECTURE.md` |
...
Le tri étant purement alphabétique sur le texte de `.card-title`, **le titre d'une carte peut volontairement différer du libellé de l'onglet correspondant** pour obtenir la position voulue dans le groupe. Cas existant : la carte « **Interférences 1D** » (l'onglet, lui, s'intitule « Interférences **en** 1D ») — le chiffre `1` se classe avant la lettre `d`, la carte passe donc avant « Interférences d'ondes de surface » et « Interférences lumineuses », alors que « Interférences en 1D » serait arrivée en dernier.
...
| `interferences-1d.jpg` | Interférences 1D |
